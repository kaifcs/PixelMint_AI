...
-- 2. Add public_id columns to processed_images if missing
ALTER TABLE public.processed_images ADD COLUMN IF NOT EXISTS original_public_id TEXT;
ALTER TABLE public.processed_images ADD COLUMN IF NOT EXISTS processed_public_id TEXT;
...
DROP POLICY IF EXISTS "Users can delete their own processed images" ON public.processed_images;
...
-- 7. Atomic RPC: Process Payment Webhook Idempotently & Upgrade Plan
...
BEGIN
  ...
  SELECT event_id INTO v_existing_event FROM public.processed_webhooks WHERE event_id = p_event_id;
  IF FOUND THEN
  ...
  IF p_payment_id IS NOT NULL THEN
    SELECT payment_id INTO v_existing_payment FROM public.processed_webhooks WHERE payment_id = p_payment_id;
    IF FOUND THEN
  ...
  INSERT INTO public.processed_webhooks (event_id, payment_id, order_id, status)
  VALUES (p_event_id, p_payment_id, p_order_id, 'processed');

  -- Update order status to paid
  UPDATE public.payment_orders
  SET status = 'paid', razorpay_payment_id = COALESCE(p_payment_id, razorpay_payment_id), updated_at = timezone('utc', now())
  WHERE razorpay_order_id = p_order_id;

  -- Upgrade profile plan to PRO
  IF p_user_id IS NOT NULL THEN
    UPDATE public.profiles
    SET plan = 'PRO', updated_at = timezone('utc', now())
    WHERE id = p_user_id;
  END IF;
...
-- 8. Atomic RPC: Quota Check and Record Image Processing
...
-- 9. Atomic RPC: Account Cascade Deletion returning Cloudinary URLs and IDs
CREATE OR REPLACE FUNCTION public.delete_user_account_atomic(p_user_id UUID)
RETURNS TABLE(original_url TEXT, original_pub_id TEXT, processed_url TEXT, processed_pub_id TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Return URLs and public IDs before deleting rows
  RETURN QUERY
  SELECT original_image_url, original_public_id, processed_image_url, processed_public_id
  FROM public.processed_images
  WHERE user_id = p_user_id;

  -- Cascade delete database records
  DELETE FROM public.processed_images WHERE user_id = p_user_id;
  DELETE FROM public.payment_orders WHERE user_id = p_user_id;
